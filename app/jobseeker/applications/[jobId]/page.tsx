...
export default function ApplicationJobDetailPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  useEffect(() => {
    const fetchApplicationData = async () => {
      try {
        ...
        const applicationsResponse = await axios.get('http://localhost:4000/api/v1/applications/user', {
          headers: {
        ...
        if (application.jobId && typeof application.jobId === 'object') {
          ...
          try {
            const jobResponse = await axios.get(`http://localhost:4000/api/v1/jobs/${application.jobId}`, {
              headers: {
                'Authorization': `Bearer ${token}`,
              }
            });
  ...
  const handleWithdrawApplication = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`http://localhost:4000/api/v1/applications/${jobId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      });
